...
static const uint8 gpioToFn[16] = {0x34, 0x18, 0x38, 0x14, 0x3C, 0x40, 0x1C, 0x20, 0x24, 0x28, 0x2C, 0x30, 0x04, 0x08, 0x0C, 0x10};
#define GPF(p) ESP8266_REG(0x800 + gpioToFn[(p & 0xF)])
...
extern int ICACHE_FLASH_ATTR gpioRead(uint8 pin)
{
  if(pin < 16){
    return GPIP(pin);
  } else if(pin == 16){
